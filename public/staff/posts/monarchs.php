...
if(is_post_request()) {
  ...
  if($result === true) {
    ...
    redirect_to(url_for('/staff/posts/monarchs.php'));
  } else {
    // show errors
...
<?php $page_title = 'Monarchs Posts'; ?>
<?php include(SHARED_PATH . '/staff_header.php'); ?>

<div id="content">
  <div class="tasks listing mb-5">
    <h1>Real Monarchs Highlighed Posts</h1>
    <?php echo display_errors($post->errors); ?>
      ...
      <?php foreach($highlighed as $highlight) { ?>
        <tr>
          <form action="<?php echo 'monarchs.php?id=' . h(u($highlight->id)) ; ?>" method="post">
  ...
  <div class="tasks listing">
    ...
    <?php echo display_errors($post->errors); ?>
      ...
      <?php foreach($posts as $post) { ?>
        <tr>
          <form action="<?php echo 'monarchs.php?id=' . h(u($post->id)) ; ?>" method="post">
...
$url = url_for('/staff/posts/monarhcs.php');
