...
<?php
  ...
  $per_page = 20;
  $total_count = Feed::count_team(NULL);
...
$sql .= "OR team IS NULL AND team_hero=1 ";
$sql .= "ORDER BY pubDate DESC ";
...
if(is_post_request()) {
  ...
  if($result === true) {
    ...
    redirect_to(url_for('/staff/posts/team.php'));
  } else {
    // show errors
  ...
  <div class="tasks listing mb-5">
    ...
    <?php echo display_errors($post->errors); ?>
      ...
      <?php foreach($highlighed as $highlight) { ?>
        <tr>
          <form action="<?php echo 'team.php?id=' . h(u($highlight->id)) ; ?>" method="post">
  ...
  <div class="tasks listing">
    ...
    <?php echo display_errors($post->errors); ?>
      ...
      <?php foreach($posts as $post) { ?>
        <tr>
          <form action="<?php echo 'team.php?id=' . h(u($post->id)) ; ?>" method="post">
...
$url = url_for('/staff/posts/team.php');
