...
if(is_post_request()) {
  ...
  if($result === true) {
    ...
    redirect_to(url_for('/staff/posts/urfc.php'));
  } else {
    // show errors
...
<div id="content">
  <div class="tasks listing mb-5">
    ...
    <?php echo display_errors($post->errors); ?>
      ...
      <?php foreach($highlighed as $highlight) { ?>
        <tr>
          <form action="<?php echo 'urfc.php?id=' . h(u($highlight->id)) ; ?>" method="post">
  ...
  <div class="tasks listing">
    <h1>All Utah Royals FC Posts</h1>
    <?php echo display_errors($post->errors); ?>
  	<div class="table-responsive">
      <table class="list table">
      <tr>
      ...
      <?php foreach($posts as $post) { ?>
        <tr>
          <form action="<?php echo 'urfc.php?id=' . h(u($post->id)) ; ?>" method="post">
...
$url = url_for('/staff/posts/urfc.php');
